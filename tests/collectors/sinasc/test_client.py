...
from include.collectors.sinasc import client
...
def test_list_arquivos_encontra_o_arquivo_do_ano(monkeypatch):
    fake_ftp = FakeFTP(["DNRJ2022.dbc", "DNRJ2021.dbc", "outrofile.txt"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)

    assert client.list_arquivos("RJ", 2022) == ["DNRJ2022.dbc"]


def test_list_arquivos_rejeita_quando_nao_encontra_nenhum_arquivo(monkeypatch):
    fake_ftp = FakeFTP(["DNRJ2021.dbc"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)

    with pytest.raises(RuntimeError, match="DNRJ2022"):
        client.list_arquivos("RJ", 2022)


def test_fetch_nascidos_vivos_marca_arquivo_origem(monkeypatch):
    fake_ftp = FakeFTP(["DNRJ2022.dbc"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)
    monkeypatch.setattr(client, "dbc2dbf", lambda origem, destino: None)

    dataframes = {
        "DNRJ2022.dbc": pd.DataFrame([{"CODMUNNASC": "330455"}]),
    }
    ...
    registros = client.fetch_nascidos_vivos("RJ", 2022)

    assert len(registros) == 1
    assert registros[0]["_arquivo_origem"] == "DNRJ2022.dbc"
