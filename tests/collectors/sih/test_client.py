...
from include.collectors.sih import client
...
def test_list_arquivos_encontra_o_arquivo_da_competencia(monkeypatch):
    fake_ftp = FakeFTP(["RDRJ2512.dbc", "RDRJ2511.dbc", "outrofile.txt"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)

    assert client.list_arquivos("RJ", 2025, 12) == ["RDRJ2512.dbc"]


def test_list_arquivos_rejeita_quando_nao_encontra_nenhum_arquivo(monkeypatch):
    fake_ftp = FakeFTP(["RDRJ2511.dbc"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)

    with pytest.raises(RuntimeError, match="RDRJ2512"):
        client.list_arquivos("RJ", 2025, 12)


def test_fetch_internacoes_marca_arquivo_origem(monkeypatch):
    fake_ftp = FakeFTP(["RDRJ2512.dbc"])
    monkeypatch.setattr(client.ftplib, "FTP", fake_ftp)
    monkeypatch.setattr(client, "dbc2dbf", lambda origem, destino: None)

    dataframes = {
        "RDRJ2512.dbc": pd.DataFrame(
            [{"N_AIH": "3325100954329", "MUNIC_MOV": "330455"}]
        ),
    }
    ...
    registros = client.fetch_internacoes("RJ", 2025, 12)

    assert len(registros) == 1
    assert registros[0]["_arquivo_origem"] == "RDRJ2512.dbc"
